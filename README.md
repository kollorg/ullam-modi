# @kollorg/ullam-modi [![NPM version](https://img.shields.io/npm/v/@kollorg/ullam-modi.svg?style=flat)](https://www.npmjs.com/package/@kollorg/ullam-modi) [![NPM monthly downloads](https://img.shields.io/npm/dm/@kollorg/ullam-modi.svg?style=flat)](https://npmjs.org/package/@kollorg/ullam-modi) [![NPM total downloads](https://img.shields.io/npm/dt/@kollorg/ullam-modi.svg?style=flat)](https://npmjs.org/package/@kollorg/ullam-modi)  [![Tests](https://github.com/kollorg/ullam-modi/actions/workflows/test.yml/badge.svg)](https://github.com/kollorg/ullam-modi/actions/workflows/test.yml)
...
- [Sponsors](#sponsors)
  ...
  * [Why use @kollorg/ullam-modi?](#why-use-@kollorg/ullam-modi)
    + [Matching features](#matching-features)
  * [Switching to @kollorg/ullam-modi](#switching-to-@kollorg/ullam-modi)
...
```sh
$ npm install --save @kollorg/ullam-modi
```
...
```js
const @kollorg/ullam-modi = require('@kollorg/ullam-modi');
// @kollorg/ullam-modi(list, patterns[, options]);
```

The [main export](#@kollorg/ullam-modi) takes a list of strings and one or more glob patterns:

```js
console.log(@kollorg/ullam-modi(['foo', 'bar', 'baz', 'qux'], ['f*', 'b*'])) //=> ['foo', 'bar', 'baz']
console.log(@kollorg/ullam-modi(['foo', 'bar', 'baz', 'qux'], ['*', '!b*'])) //=> ['foo', 'qux']
```

Use [.isMatch()](#ismatch) to for boolean matching:

```js
console.log(@kollorg/ullam-modi.isMatch('foo', 'f*')) //=> true
console.log(@kollorg/ullam-modi.isMatch('foo', ['b*', 'f*'])) //=> true
```

[Switching](#switching-to-@kollorg/ullam-modi) from minimatch and multimatch is easy!

<br>

## Why use @kollorg/ullam-modi?

> @kollorg/ullam-modi is a [replacement](#switching-to-@kollorg/ullam-modi) for minimatch and multimatch
...
## Switching to @kollorg/ullam-modi

_(There is one notable difference between @kollorg/ullam-modi and minimatch in regards to how backslashes are handled. See [the notes about backslashes](#backslashes) for more information.)_

### From minimatch

Use [@kollorg/ullam-modi.isMatch()](#ismatch) instead of `minimatch()`:

```js
console.log(@kollorg/ullam-modi.isMatch('foo', 'b*')); //=> false
```

Use [@kollorg/ullam-modi.match()](#match) instead of `minimatch.match()`:

```js
console.log(@kollorg/ullam-modi.match(['foo', 'bar'], 'b*')); //=> 'bar'
```
...
```js
console.log(@kollorg/ullam-modi(['foo', 'bar', 'baz'], ['f*', '*z'])); //=> ['foo', 'baz']
```
...
```js
const mm = require('@kollorg/ullam-modi');
...
```js
var mm = require('@kollorg/ullam-modi');
...
```js
const { braces } = require('@kollorg/ullam-modi');
...
```js
@kollorg/ullam-modi(['a/b.js', 'a/c.md'], '*.js');
//=> []

@kollorg/ullam-modi(['a/b.js', 'a/c.md'], '*.js', { basename: true });
//=> ['a/b.js']
```
...
```js
const files = ['abc', 'ajz'];
console.log(@kollorg/ullam-modi(files, '[a-c]*'));
//=> ['abc', 'ajz']

console.log(@kollorg/ullam-modi(files, '[a-c]*', { bash: false }));
```
...
const regex = @kollorg/ullam-modi.makeRe('foo/{01..25}/bar', {
  expandRange(a, b) {
...
```js
const isMatch = @kollorg/ullam-modi.matcher('*', { ignore: 'f*' });
...
```js
console.log(@kollorg/ullam-modi(['a/z', 'a/b', 'a/!(z)'], 'a/!(z)'));
//=> ['a/b', 'a/!(z)']

console.log(@kollorg/ullam-modi(['a/z', 'a/b', 'a/!(z)'], 'a/!(z)', { noextglob: true }));
...
```js
@kollorg/ullam-modi(['a/b', 'a/b/c', 'a/b/c/d'], 'a/**');
//=> ['a/b', 'a/b/c', 'a/b/c/d']

@kollorg/ullam-modi(['a/b', 'a/b/c', 'a/b/c/d'], 'a/**', {noglobstar: true});
//=> ['a/b']
```
...
const isMatch = @kollorg/ullam-modi.matcher('*', { onIgnore, ignore: 'f*' });
isMatch('foo');
isMatch('bar');
isMatch('baz');
```
...
const isMatch = @kollorg/ullam-modi.matcher('**', { onMatch, posixSlashes: true });
isMatch('some\\path');
isMatch('some\\path');
isMatch('some\\path');
```
...
const isMatch = @kollorg/ullam-modi('*', { onResult, ignore: 'f*' });
isMatch('foo');
isMatch('bar');
isMatch('baz');
```
...
```js
console.log(@kollorg/ullam-modi.match(['a\\b\\c'], 'a/**'));
//=> ['a/b/c']

console.log(@kollorg/ullam-modi.match(['a\\b\\c'], { posixSlashes: false }));
//=> ['a\\b\\c']
```
...
```js
console.log(@kollorg/ullam-modi.match(['abc', 'a\\*c'], 'a\\*c'));
//=> ['a\\*c']

console.log(@kollorg/ullam-modi.match(['abc', 'a\\*c'], 'a\\*c', { unescape: true }));
//=> ['a*c']
```
...
```js
console.log(@kollorg/ullam-modi.isMatch('a1', '[[:alpha:][:digit:]]')) //=> true
console.log(@kollorg/ullam-modi.isMatch('a1', '[[:alpha:][:alpha:]]')) //=> false
```
...
However, it's suprising how many edge cases and rabbit holes there are with glob matching, and since there is no real glob specification, and @kollorg/ullam-modi is more accurate than both Bash and minimatch, there are cases where best-guesses were made for behavior. In a few cases where Bash had no answers, we used wildmatch (used by git) as a fallback.

### Backslashes

There is an important, notable difference between minimatch and @kollorg/ullam-modi _in regards to how backslashes are handled_ in glob patterns.
...
We made this decision for @kollorg/ullam-modi for a couple of reasons:
...
Note that when you pass something like `path.join('foo', '*')` to @kollorg/ullam-modi, you are creating a filepath and expecting it to still work as a glob pattern. This causes problems on windows, since the `path.sep` is `\\`.

In other words, since `\\` is reserved as an escape character in globs, on windows `path.join('foo', '*')` would result in `foo\\*`, which tells @kollorg/ullam-modi to match `*` as a literal character. This is the same behavior as bash.
...
```sh
# .makeRe star
  @kollorg/ullam-modi x 2,232,802 ops/sec ±2.34% (89 runs sampled))
  minimatch x 781,018 ops/sec ±6.74% (92 runs sampled))

# .makeRe star; dot=true
  @kollorg/ullam-modi x 1,863,453 ops/sec ±0.74% (93 runs sampled)
  minimatch x 723,105 ops/sec ±0.75% (93 runs sampled)

# .makeRe globstar
  @kollorg/ullam-modi x 1,624,179 ops/sec ±2.22% (91 runs sampled)
  minimatch x 1,117,230 ops/sec ±2.78% (86 runs sampled))

# .makeRe globstars
  @kollorg/ullam-modi x 1,658,642 ops/sec ±0.86% (92 runs sampled)
  minimatch x 741,224 ops/sec ±1.24% (89 runs sampled))

# .makeRe with leading star
  @kollorg/ullam-modi x 1,525,014 ops/sec ±1.63% (90 runs sampled)
  minimatch x 561,074 ops/sec ±3.07% (89 runs sampled)

# .makeRe - braces
  @kollorg/ullam-modi x 172,478 ops/sec ±2.37% (78 runs sampled)
  minimatch x 96,087 ops/sec ±2.34% (88 runs sampled)))

# .makeRe braces - range (expanded)
  @kollorg/ullam-modi x 26,973 ops/sec ±0.84% (89 runs sampled)
  minimatch x 3,023 ops/sec ±0.99% (90 runs sampled))

# .makeRe braces - range (compiled)
  @kollorg/ullam-modi x 152,892 ops/sec ±1.67% (83 runs sampled)
  minimatch x 992 ops/sec ±3.50% (89 runs sampled)d))

# .makeRe braces - nested ranges (expanded)
  @kollorg/ullam-modi x 15,816 ops/sec ±13.05% (80 runs sampled)
  minimatch x 2,953 ops/sec ±1.64% (91 runs sampled)

# .makeRe braces - nested ranges (compiled)
  @kollorg/ullam-modi x 110,881 ops/sec ±1.85% (82 runs sampled)
  minimatch x 1,008 ops/sec ±1.51% (91 runs sampled)

# .makeRe braces - set (compiled)
  @kollorg/ullam-modi x 134,930 ops/sec ±3.54% (63 runs sampled))
  minimatch x 43,242 ops/sec ±0.60% (93 runs sampled)

# .makeRe braces - nested sets (compiled)
  @kollorg/ullam-modi x 94,455 ops/sec ±1.74% (69 runs sampled))
...
It's important to us that @kollorg/ullam-modi work consistently on all platforms. If you encounter any platform-specific matching or path related issues, please let us know (pull requests are also greatly appreciated).
...
[brackets]: https://github.com/@kollorg/ullam-modi/expand-brackets
[braces]: https://github.com/@kollorg/ullam-modi/braces
...
- [braces](https://www.npmjs.com/package/braces): Bash-like brace expansion, implemented in JavaScript. Safer than other brace expansion libs, with complete support… [more](https://github.com/@kollorg/ullam-modi/braces) | [homepage](https://github.com/@kollorg/ullam-modi/braces "Bash-like brace expansion, implemented in JavaScript. Safer than other brace expansion libs, with complete support for the Bash 4.3 braces specification, without sacrificing speed.")
- [expand-brackets](https://www.npmjs.com/package/expand-brackets): Expand POSIX bracket expressions (character classes) in glob patterns. | [homepage](https://github.com/@kollorg/ullam-modi/expand-brackets "Expand POSIX bracket expressions (character classes) in glob patterns.")
- [extglob](https://www.npmjs.com/package/extglob): Extended glob support for JavaScript. Adds (almost) the expressive power of regular expressions to glob… [more](https://github.com/@kollorg/ullam-modi/extglob) | [homepage](https://github.com/@kollorg/ullam-modi/extglob "Extended glob support for JavaScript. Adds (almost) the expressive power of regular expressions to glob patterns.")
- [fill-range](https://www.npmjs.com/package/fill-range): Fill in a range of numbers or letters, optionally passing an increment or `step` to… [more](https://github.com/jonschlinkert/fill-range) | [homepage](https://github.com/jonschlinkert/fill-range "Fill in a range of numbers or letters, optionally passing an increment or `step` to use, or create a regex-compatible range with `options.toRegex`")
- [nanomatch](https://www.npmjs.com/package/nanomatch): Fast, minimal glob matcher for node.js. Similar to @kollorg/ullam-modi, minimatch and multimatch, but complete Bash… [more](https://github.com/@kollorg/ullam-modi/nanomatch) | [homepage](https://github.com/@kollorg/ullam-modi/nanomatch "Fast, minimal glob matcher for node.js. Similar to @kollorg/ullam-modi, minimatch and multimatch, but complete Bash 4.3 wildcard support only (no support for exglobs, posix brackets or braces)")
...
[extglob]: https://github.com/@kollorg/ullam-modi/extglob
